...
- Upload storage is now auto-detecting (Aug 29, 2026): if `EMERGENT_LLM_KEY` is present the file goes to Emergent object storage (`/api/upload/media/{id}`); otherwise (user's own self-hosted 2-core server) it saves to `backend/uploads/{type}s/` and returns `/api/uploads/{type}s/{id}` served by the existing static mount. Object-storage failures at runtime also fall back to local disk. Test: `python backend/tests/test_upload_fallback.py`.
- Phone validation restored in the ACTIVE send flow (Aug 29, 2026): `frontend/src/utils/phoneValidator.js` (libphonenumber-js) + `useRecipients` now auto-normalize on paste/Excel upload — invalid numbers are dropped with an amber "N invalid number(s) removed" panel, and local numbers get the selected country's dial code. RecipientUploader has a searchable country dropdown (245 countries, India/UAE/US first) and a "Re-check with +NN" button. Backend `normalize_phone_number` fixed: no longer skips prefixing 10-digit numbers that merely start with the country code (91 bug).
- Large-campaign fix (Aug 29, 2026): posting 30k recipients in one request failed ("Failed to send messages") because a ~1.3MB body exceeds default proxy body limits / axios 30s timeout. Added a chunked create flow: `POST /api/messages/campaigns/init` (validations + draft campaign) → `POST /api/messages/campaigns/{id}/recipients` in 1,000-row chunks → `POST /api/messages/campaigns/{id}/start`. Frontend auto-uses it above 1,000 recipients and shows "Uploading X / Y..." on the button. Draft campaigns are hidden from lists/summary. Verified: 30k uploaded in ~5s via 30 chunks; 2,500-recipient UI run created a scheduled drip campaign correctly.
